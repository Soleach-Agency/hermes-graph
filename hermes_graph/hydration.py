...
def _rows_from_store(path: Path) -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        rows = connection.execute(
            f"SELECT {selected} FROM sessions ORDER BY started_at, id"
        ).fetchall()
...
def hydrate_runtime(
    runtime_stores: dict[str, Path] | None = None, *, profile_name: str | None = None
) -> dict[str, int]:
    """Project current runtime sessions through the normal scene mutation history.

    Unsupported or unavailable stores are ignored. The stored projection is scoped
    to this adapter, so a later startup only removes stale state it created itself.
    """
    stores = (
        runtime_stores
        if runtime_stores is not None
        else discover_runtime_stores(profile_name=profile_name)
    )
    ...
    for profile, row in records:
        ...
        existing_profile = profiles_by_session.get(session_id, profile)
        profiles_by_session[session_id] = (
            profile if existing_profile == profile else None
        )
    ...
    for profile, row in records:
        ...
        nodes[agent] = {
            "id": agent,
            "kind": "subagent" if _is_delegate(row) else "agent",
            "label": str(row.get("model") or "Hermes agent"),
            "status": status,
            "color": None,
            "size": None,
            "pressure": None,
        ...
        edges[f"belongs:{agent}:{session}"] = {
            "id": f"belongs:{agent}:{session}",
            "source": agent,
            "target": session,
            "kind": "belongs_to",
            "active": True,
            "metadata": {"provenance": _RUNTIME_PROVENANCE},
        }
        hydrated_sessions += 1
    ...
    for profile, row in records:
        ...
        session = _id("session", session_id)
        edges[f"parent:{parent_session}:{session}"] = {
            "id": f"parent:{parent_session}:{session}",
        ...
        if _is_delegate(row) and parent_profile is not None:
            ...
            agent = _id("agent", f"runtime:{profile}:{session_id}")
            edges[f"spawned:{parent_agent}:{agent}"] = {
                "id": f"spawned:{parent_agent}:{agent}",
                "source": parent_agent,
                "target": agent,
                "kind": "spawned",
                "active": True,
    ...
    replace_runtime_hydration_projection(
        _RUNTIME_PROVENANCE, list(nodes.values()), list(edges.values())
    )
    return {"hydratedSessions": hydrated_sessions, "stores": supported_stores}
